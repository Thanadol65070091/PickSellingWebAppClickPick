...
export async function getOrCreateDraftOrder(userId) {
  const found = await query('SELECT * FROM orders WHERE user_id=$1 AND status=$2', [userId, 'draft']);
  if (found.rows[0]) return found.rows[0];
  const { rows } = await query(
    `INSERT INTO orders (user_id, status, total, delivery_fee, discount, billing_detail)
     VALUES ($1,'draft',0,$2,$3,'{}'::jsonb)
     RETURNING *`, [userId, DELIVERY_FEE, DISCOUNT]
  );
  return rows[0];
}
...
export async function addItem(userId, { product_id, quantity, customization }) {
  if (!product_id || !quantity) throw new Error('product_id and quantity required');
  const order = await getOrCreateDraftOrder(userId);
  const { rows: prodRows } = await query('SELECT * FROM products WHERE id=$1', [product_id]);
  const product = prodRows[0];
  if (!product) throw new Error('Product not found');
  if (quantity < 1) throw new Error('Invalid quantity');
  if (quantity > product.stock) throw new Error('Quantity exceeds stock');
  if (product.category === 'custompick') {
    if (!validateCustomization(customization)) throw new Error('Invalid customization');
  } else {
    if (customization) throw new Error('Customization not allowed for this product');
  }
  ...
  if (product.category === 'custompick' && customization) {
    ...
    await query('UPDATE order_items SET customization_id=$1 WHERE id=$2', [custRows[0].id, item.id]);
  }
...
export async function updateItem(userId, itemId, { quantity, customization }) {
  const { rows: itemRows } = await query(
    `SELECT oi.*, p.category, p.stock FROM order_items oi JOIN products p ON p.id=oi.product_id WHERE oi.id=$1`, [itemId]
  );
  const item = itemRows[0];
  if (!item) throw new Error('Item not found');
  // Ensure item belongs to user's draft order
  const { rows: orderRows } = await query('SELECT * FROM orders WHERE id=$1', [item.order_id]);
  ...
  if (quantity !== undefined) {
    ...
    await query('UPDATE order_items SET quantity=$1 WHERE id=$2', [quantity, itemId]);
  }
  if (item.category === 'custompick') {
    if (!validateCustomization(customization)) throw new Error('Invalid customization');
    const { rows: custRows } = await query('SELECT * FROM customization WHERE order_item_id=$1', [itemId]);
    if (custRows[0]) {
      await query('UPDATE customization SET shape=$1,color=$2,text=$3 WHERE order_item_id=$4', [customization.shape, customization.color, customization.text, itemId]);
    } else {
      const { rows } = await query(
      ...
      await query('UPDATE order_items SET customization_id=$1 WHERE id=$2', [rows[0].id, itemId]);
    }
...
export async function deleteItem(userId, itemId) {
  ...
  const { rows: itemRows } = await query('SELECT * FROM order_items WHERE id=$1', [itemId]);
  const item = itemRows[0];
  if (!item) return;
  const { rows: orderRows } = await query('SELECT * FROM orders WHERE id=$1', [item.order_id]);
  const order = orderRows[0];
...
export async function recomputeTotals(orderId) {
  const { rows } = await query('SELECT quantity, price FROM order_items WHERE order_id=$1', [orderId]);
  const subtotal = rows.reduce((s, r) => s + Number(r.quantity) * Number(r.price), 0);
  const total = subtotal + DELIVERY_FEE - DISCOUNT;
  await query('UPDATE orders SET total=$1, delivery_fee=$2, discount=$3 WHERE id=$4', [total, DELIVERY_FEE, DISCOUNT, orderId]);
}

export async function checkout(userId, billing_detail) {
  const order = await getOrCreateDraftOrder(userId);
  await recomputeTotals(order.id);
  const { rows } = await query('SELECT COUNT(*)::int as cnt FROM order_items WHERE order_id=$1', [order.id]);
  if (rows[0].cnt === 0) throw new Error('Cart is empty');
  await query('UPDATE orders SET billing_detail=$1, status=$2 WHERE id=$3', [billing_detail, 'pending', order.id]);
...
export async function payment(orderId) {
  ...
  const order = orderRows[0];
  if (!order || order.status !== 'pending') throw new Error('Order not pending');
  const { rows: items } = await query(
    `SELECT oi.*, p.stock FROM order_items oi JOIN products p ON p.id=oi.product_id WHERE oi.order_id=$1`, [orderId]
  );
  ...
  // decrement stock
  for (const it of items) {
    await query('UPDATE products SET stock=stock-$1 WHERE id=$2', [it.quantity, it.product_id]);
  }
...
export async function listOrdersForUser(user) {
  ...
  const { rows } = await query('SELECT * FROM orders WHERE user_id=$1 ORDER BY created_at DESC', [user.sub]);
  return rows;
}
...
export async function markShipped(id) {
  const { rows } = await query('UPDATE orders SET status=$1 WHERE id=$2 RETURNING *', ['shipped', id]);
  return rows[0];
}

export async function markDelivered(id) {
  const { rows } = await query('UPDATE orders SET status=$1 WHERE id=$2 RETURNING *', ['delivered', id]);
  return rows[0];
}
